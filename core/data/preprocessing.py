...
class DataProcessor:
    ...
    def _load_dataset(self) -> Union[Dataset, DatasetDict]:
        ...
        if self.data_path:
            ...
            logger.info(
                f"Loading dataset from Hugging Face: {self.dataset_name}, {self.dataset_config}"
            )
            return load_dataset(self.dataset_name, self.dataset_config)
        else:
            raise ValueError(
                "Either 'data_path' or 'dataset_name' and 'dataset_config' must be provided."
            )
    ...
    def _setup_char_augmenter(self) -> List[char_augmenter.CharAugmenter]:
        ...
        char_swap_probability = self.augmentation_config.get(
            "char_swap_probability", 0.5
        )
    ...
    def _setup_word_augmenter(self) -> List[word_augmenter.WordAugmenter]:
        ...
        word_swap_probability = self.augmentation_config.get(
            "word_swap_probability", 0.5
        )

        word_swap_augmenter = word_augmenter.RandomWordAug(
            action="swap",
            aug_p=word_swap_ratio,
            aug_min=1,
            aug_max=2,
        ...
        return [
            word_swap_augmenter,
            word_delete_augmenter,
        ]  # Remove word_insert_augmenter from the list
    ...
    def _setup_sentence_augmenter(self) -> List[sentence_augmenter.SentenceAugmenter]:
        ...
        sentence_delete_ratio = self.augmentation_config.get(
            "sentence_delete_ratio", 0.1
        )
        sentence_insert_ratio = self.augmentation_config.get(
            "sentence_insert_ratio", 0.1
        )
        sentence_swap_probability = self.augmentation_config.get(
    ...
    def _setup_synonym_augmenter(self) -> word_augmenter.SynonymAug:
        ...
        """
        synonym_augmentation_probability = self.augmentation_config.get(
            "synonym_augmentation_probability", 0.1
        )
    ...
    def _setup_backtranslation_augmenter(self) -> BackTranslationAug:
        ...
        backtranslation_language = self.augmentation_config.get(
            "backtranslation_language", "fr"
        )
    ...
    def load_and_process_dataset(self) -> Union[Dataset, DatasetDict]:
        ...
        with tqdm(
            total=len(dataset), desc="Processing Dataset", leave=False
        ) as pbar_splits:
            for split in dataset.keys():
                logger.info(f"✨ Preprocessing {split} split... ✨")
                processed_data = {
                    "text": list(self._preprocess_split(dataset[split], split))
                }  # Pass pbar to _preprocess_split
                processed_dataset[split] = Dataset.from_dict(processed_data)
                pbar_splits.update(
                    1
                )  # Update the progress bar after processing each split

        logger.info("✅ Dataset preprocessing complete! 🎉")
        return processed_dataset

    def _preprocess_split(
        self, dataset_split: Dataset, split: str
        ...
        """
        logger.info(
            f"✨ Preprocessing {split} split... ✨"
        )  # Log split preprocessing start
        ...
        with tqdm(
            ...
            for i in range(0, len(dataset_split), batch_size):
                batch_cache_path = os.path.join(
                    self.cache_dir, f"{split}_batch_{i}.arrow"
                )
                if os.path.exists(batch_cache_path):
                    ...
                    processed_batch_dataset = Dataset.from_dict(
                        {"text": processed_batch}
                    )
    ...
    def _preprocess_batch(
        self, batch: Dict[str, Union[str, List[str]]]
    ) -> Generator[str, None, None]:
        """
        ...
        texts = [
            " ".join(text) if isinstance(text, list) else text for text in batch["text"]
        ]
        ...
        for i in range(0, len(texts), chunk_size):
            ...
            texts_series = texts_series.str.replace(
                r"[^\w\s]", "", regex=True
            )  # Remove punctuation
