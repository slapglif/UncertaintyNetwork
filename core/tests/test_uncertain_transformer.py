...
def test_uncertain_transformer():
    ...
    check_layer(model.transformer.rotary_pos_emb, (2, 10, config.d_model),
    ...
    check_layer(model.transformer.final_layer_norm, (2, 10, config.d_model), (2, 10, config.d_model))
...
def check_model_learnability(model: nn.Module, input_shape: Tuple[int, ...], device: torch.device, num_steps: int = 5):
    """
    ...
    for step in range(num_steps):
        ...
        if step == 0:
            ...
            assert loss.item() < initial_loss, f"Loss did not decrease: initial {initial_loss:.4f}, final {loss.item():.4f}"
...
def test_transformer_encoder_layer():
    ...
    transformer = TransformerEncoderLayer(config).to("cuda")
    check_layer(transformer, (2, 10, config.d_model), (2, 10, config.d_model))
...
def test_kan_feed_forward():
    ...
    kan_ff = KANFeedForward(config).to("cuda")
    check_layer(kan_ff, (2, 10, config.d_model), (2, 10, config.d_model))
